...
def get_nasdaq_close(date):
    ...
    data = yf.download("^IXIC", start=date, end=next_day.strftime('%Y-%m-%d'), progress=False)
    if not data.empty:
...
def draw_bitcoin_user_inputs(defaults):
    ...
    with col1:
        ...
        X_live["Nasdaq_Close"] = st.number_input("Nasdaq Index", value=defaults.get("Nasdaq_Close"))

    with col2:
        X_live["CPI"] = st.number_input("Consumer Price Index", value=defaults.get("CPI"))
        X_live["Retail_Sales"] = st.number_input("Retail Sales (USD)", value=defaults.get("Retail_Sales"))

    with col3:
        X_live["M2_Money_Supply"] = st.number_input("M2 Money Supply (USD)", value=defaults.get("M2_Money_Supply"))
        X_live["Real_GDP"] = st.number_input("Real GDP (USD)", value=defaults.get("Real_GDP"))

    st.caption("Macroeconomic indicators are automatically pulled from [FRED](https://fred.stlouisfed.org) based on the selected date. Visit the site for more info.")
